...
public class ContextProcessor {
  ...
  public static synchronized ContextProcessor getInstance() {
    if (_instance == null) {
  ...
  public static synchronized ContextProcessor newInstance() {
    return new ContextProcessor(
        ServiceLoader.load(ContextHandler.class).stream()
  ...
  public void resolveForJsonConversion(
      final JsonGenerator jsonGenerator, final Map<String, String> allNamespaces) {
    // Iterate and find the matching handler such as GS1 Egypt or Default during the XML -> JSON
    // conversion
