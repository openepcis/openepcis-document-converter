...
public class DocumentConverterServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (from.isEmpty()) {
      servletSupport.writeException(
          new BadRequestException("unable to detect EPCISFormat"), accept.get(), resp);
      return;
    }
    try {
      if (req.getRequestURI().endsWith(CONVERT_JSON_2_0)) {
        ...
        runVersionTransformer(
            req, resp, from.get(), EPCISFormat.JSON_LD, EPCISVersion.VERSION_2_0_0);
      } else {
  ...
  private void runVersionTransformer(
