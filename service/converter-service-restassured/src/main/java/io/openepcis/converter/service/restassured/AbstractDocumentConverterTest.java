...
public abstract class AbstractDocumentConverterTest {
  ...
  /**
   * Disabled Test for non-functional XML 1.2 -> JSON conversion with GS1-EPC-Format
  ...
  public void xmlConverter_1_2_ValidRequestEPCFormatTest() throws Exception {
    ...
    Assertions.assertEquals(
        1,
        response.getBody().jsonPath().getList("epcisBody.eventList", HashMap.class).stream()
            .filter(eventAsMap -> eventAsMap.get("type").equals("ObjectEvent"))
            .count());
  }
}
