              ...
              .newSchema(
                  new StreamSource(
                      EventValidator.class
                          .getClassLoader()
                          .getResourceAsStream("EPCISEventXSD.xsd")));

    } catch (SAXException e) {
      throw new EventFormatConversionException(e);
    }
  }
    ...
    if (event instanceof StringWriter) {
      ...
      try {
        ...
        if (parent != null && parent.textValue() != null) {
          ...
          switch (epcisEvent) {
            case "ObjectEvent":
              schemaFile = "ObjectEventSchema.json";
              break;
            case "AggregationEvent":
              schemaFile = "AggregationEventSchema.json";
              break;
            case "TransactionEvent":
              schemaFile = "TransactionEventSchema.json";
              break;
            case "TransformationEvent":
              schemaFile = "TransformationEventSchema.json";
              break;
            case "AssociationEvent":
              schemaFile = "AssociationEventSchema.json";
              break;
            default:
