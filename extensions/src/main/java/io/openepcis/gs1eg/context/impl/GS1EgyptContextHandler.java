...
public class GS1EgyptContextHandler implements ContextHandler {
    public static final String GS1_EGYPT_AT_CONTEXT_URL = "https://ref.gs1eg.org/extensions/epcis/hc/2.0.0/hc-context.jsonld";
    ...
    public void populateXmlNamespaces(final DefaultJsonSchemaNamespaceURIResolver defaultJsonSchemaNamespaceURIResolver) {
        defaultJsonSchemaNamespaceURIResolver.populateDocumentNamespaces(GS1_EGYPT_2_0_NAMESPACE, GS1_EGYPT_DEFAULT_PREFIX);
    }


    @Override
    public boolean isContextHandler(final Map<String, String> allNamespaces) {
        return allNamespaces.containsKey(GS1_EGYPT_AT_CONTEXT_URL) || allNamespaces.containsKey(GS1_EGYPT_2_0_NAMESPACE);
    }

    @Override
    public Map<String, URI> getContextUrls() {
        try{
            return Map.of(GS1_EGYPT_AT_CONTEXT_URL, Objects.requireNonNull(GS1EgyptContextHandler.class.getResource("/hc-context.jsonld")).toURI());
        }catch (Exception e){
            throw new FormatConverterException("Loading of GS1Egypt context file failed : " + e.getMessage(), e);
        }
    }
}
